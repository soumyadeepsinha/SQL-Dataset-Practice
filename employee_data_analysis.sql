...
LOAD DATA LOCAL INFILE '/Users/soumyadeepsinha/Documents/software/dataset/employee_data.csv'
INTO TABLE EmployeeData
FIELDS TERMINATED BY ',' 
ENCLOSED BY '"'
LINES TERMINATED BY '\n' -- Or '\r\n' if the CSV was created on a Windows machine
IGNORE 1 ROWS; -- This ignores the header row in your CSV
...
SELECT EmpID, FirstName, LastName, Title, DOB, Age, GenderCode FROM EmployeeData WHERE Age>=60;

-- Show table with dataset with specific query


-- Show table with employees - where their performance needs improvement
SELECT * FROM EmployeeData WHERE `Performance Score` = 'Needs Improvement';


-- Show male employees record
SELECT EmpID, FirstName, LastName, ADEmail from EmployeeData WHERE EmployeeStatus = 'Active' AND GenderCode = 'Male';

-- Show employees who are working till date in Software engineering dept
SELECT 
	EmpID, FirstName, LastName, Title, ADEmail, Division from EmployeeData
...
CREATE TABLE IF NOT EXISTS Training_and_Development_Data (
    ...
    TrainingDate DATE NOT NULL,              -- Better for searching dates
...
SELECT SUM(TrainingCost) FROM Training_and_Development_Data;

-- Show top 10 emoployee's details who passed during training and cost for them in Descending order
SELECT * FROM Training_and_Development_Data
	WHERE TrainingOutcome = 'Passed' AND TrainingType = 'Internal' ORDER BY TrainingCost DESC LIMIT 10;
...
FROM Training_and_Development_Data WHERE TrainingOutcome IN ('Passed', 'Completed');
...
-- Create a new view
CREATE OR REPLACE VIEW Employee_Training_Performance_Views
AS
SELECT 
	e.EmpID, 
	e.FirstName, 
...
SELECT EmpID, FirstName, LastName, 'Software Engineer' AS Designation, TrainingCost
	...
	WHERE (TrainingOutcome LIKE '%Completed%' OR TrainingOutcome LIKE '%Passed%')
